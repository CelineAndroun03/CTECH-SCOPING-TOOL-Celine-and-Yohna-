...
data["IT_norm"] = (data["Investigation_type"].astype(str).str.strip().str.upper())
it_ohe = pd.get_dummies(data["IT_norm"],prefix="IT",drop_first=False)
data = pd.concat([data, it_ohe], axis=1)
data = data.drop(columns=["IT_norm", "Investigation_type"], errors="ignore")
print("Created IT OHE columns:", list(it_ohe.columns))

# Drop columns with only 1 unique value
data = data.drop(columns = ["2 (EN 60950/EN 60065)","3 (60950-21)","5 (60950-23)","BSMI"])
